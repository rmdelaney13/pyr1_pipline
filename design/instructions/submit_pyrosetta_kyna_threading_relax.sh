...
#SBATCH --mem=8G
#SBATCH --array=1-691  # <--- UPDATE THIS NUMBER to match the count from the 'find' command below

# === Configuration ===

# 1. WHERE ARE THE ORIGINAL INPUT PDBS? (The Parents)
# Based on your logs, they are likely here:
TEMPLATE_DIR="/scratch/alpine/ryde3462/kyna_design/filtered_1"

# 2. WHERE ARE THE MPNN OUTPUTS? (The folder containing arrayXXX_mpnn folders)
MPNN_OUTPUT_BASE="/scratch/alpine/ryde3462/kyna_design/mpnn_output_2"

# 3. WHERE DO YOU WANT THE RESULTS?
OUTPUT_DIR="/scratch/alpine/ryde3462/kyna_design/relax_2"
mkdir -p "$OUTPUT_DIR"

# 4. PATHS TO SCRIPTS/PARAMS
PYTHON_SCRIPT="/projects/ryde3462/software/LigandMPNN/ligand_alignment_mpnn/scripts/instructions/kyna_relax.py"
LIGAND_PARAMS="/projects/ryde3462/kyna/conformers_deprot/0/0.params"

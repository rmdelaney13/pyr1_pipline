...
def filter_designs(df: pd.DataFrame, target_n: int, max_unsat: int, max_per_parent: int,
                   check_o1: bool, check_o2: bool, check_o3: bool, check_charge: bool) -> pd.DataFrame:
    require_cols(df)
    d = df.copy()
    ...
    if check_o3:
        mask_combined &= yes_mask(d["O3_polar_contact"])
...
def main() -> None:
    ...
    parser.add_argument("--ignore_o3", action="store_true", help="If set, O3 polar contact is NOT required")

...
#SBATCH --mem=8G
#SBATCH --array=1-282   # <--- UPDATE THIS NUMBER to match the count from the 'find' command below

# === Configuration ===

# 1. WHERE ARE THE ORIGINAL INPUT PDBS? (The Parents)
# Based on your logs, they are likely here:
TEMPLATE_DIR="/scratch/alpine/ryde3462/DCA_design_flipped/final_pass_score_repacked"

# 2. WHERE ARE THE MPNN OUTPUTS? (The folder containing arrayXXX_mpnn folders)
MPNN_OUTPUT_BASE="/scratch/alpine/ryde3462/DCA_design_flipped/mpnn_output_1"

# 3. WHERE DO YOU WANT THE RESULTS?
OUTPUT_DIR="/scratch/alpine/ryde3462/DCA_design_flipped/relax_1"
mkdir -p "$OUTPUT_DIR"

# 4. PATHS TO SCRIPTS/PARAMS
PYTHON_SCRIPT="/projects/ryde3462/software/LigandMPNN/ligand_alignment_mpnn/scripts/instructions/CDCA_relax_threaded.py"
LIGAND_PARAMS="/projects/ryde3462/bile_acids/CDCA/conformers_deprot/0/0.params"

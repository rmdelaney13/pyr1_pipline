...
# =============================================================================
# 4. SCORING HELPERS (UPDATED FOR CDCA)
# =============================================================================

def check_polar_contact_with_ligand_atom(pose, ligand_res, atom_name, max_dist=3.5):
    """
    Checks if a specific ligand atom (e.g., O1) is making a polar contact 
    (distance < 3.5A) with any Protein Nitrogen or Oxygen.
    """
    polar_atoms = {"N", "O"}
    ...
    # Iterate over all residues to find contacts
    for i in range(1, pose.total_residue() + 1):
        if i == ligand_res: continue
        res = pose.residue(i)
        
        # Skip if not protein (optional optimization)
...
def check_carboxylic_acid_satisfaction(pose, ligand_idx, receptor_chain="A",
    ...
    """
    Checks if the Carboxylic Acid (COO-) is satisfied.
    For CDCA:
      - Oxygens are likely O3 and O4.
      - Carbon is likely C24.
    """
    ...
    # Verify atoms exist in this ligand
    if not (all(ox in atoms for ox in ox_names)):
        # Fallback: if O3/O4 don't exist, try printing what does exist for debugging
        # print(f"Warning: Expected atoms {ox_names} not found in ligand.")
        return {"satisfied": False}

    ox_idx = {ox: ligand_res.atom_index(ox) for ox in ox_names if ligand_res.has(ox)}
    
    # 1. Check direct HBonds using Rosetta HBondSet
    hb_set = HBondSet()
    fill_hbond_set(pose, False, hb_set)
    hbcount = 0
    for i in range(1, hb_set.nhbonds() + 1):
        h = hb_set.hbond(i)
        # Check if acceptor is one of our Carboxyl Oxygens
        if h.acc_res() == ligand_idx and h.acc_atm() in ox_idx.values():
            d, a = h.get_HAdist(pose), h.get_AHDangle(pose)
            if d < 3.0 and a > 120:
                hbcount += 1
                
    # 2. Check Salt Bridges (Distance to ARG/LYS positive groups)
    sat_sb = False
    pdb_info = pose.pdb_info()
    ...
    # Satisfied if: At least 2 H-bonds OR a Salt Bridge exists
...
def score_and_dump(pose, outp, sf, xml_mover, ligand_idx, water_idx):
    ...
    if ligand_idx:
        ...
        # Explicitly pass CDCA atom names here just to be safe
        charge_sat = 1 if check_carboxylic_acid_satisfaction(
            pose, ligand_idx, ox_names=["O3", "O4"], carbon_name="C24"
        ).get("satisfied") else 0
    else:
...
def main():
    ...
    # Weights for design/relax
    sf.set_weight(atom_pair_constraint, 0.4)
    sf.set_weight(angle_constraint, 0.4)
